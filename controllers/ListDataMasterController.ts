...
export class ListDataMasterController{
    async CreateListDataMaster(req: Request,res: Response){
        try {            
            ...
            let returnObj = await Util.returnObj([error],statusCodes.error,'listdatamaster','createerr')
    ...
    async getAllListDataMaster(req: Request,res: Response){
        try {
            ...
            if(listdatamaster.status > 299){
                let returnObj = await Util.returnObj([listdatamaster.data],statusCodes.error,'listdatamaster','getallerr')
                return res.json(returnObj)
            }
            else{
                let returnObj = await Util.returnObj(listdatamaster.data,statusCodes.error,'listdatamaster','getall')
                return res.json(returnObj)            
            }                
        } catch (error) {
            let returnObj = await Util.returnObj([error],statusCodes.error,'listdatamaster','getallerr')
            return res.json(returnObj)        
        }
    }
